...
unsigned inputTimingSampling_ms = 30; /*!< Input timing sampling in ms */
...
void runSimulation(double k0, double kf, double step)
{
    ...
    inputV_destroy();
    outputV_destroy();
    pthread_mutex_destroy(&_k_mutex);
...
void * calculateInputV(void * arg)
{
    ...
    while ( k <= kf )
    {
        ...
        pthread_mutex_lock(&_k_mutex);
        inputVector(k); // Calculate input vector
        k += step;
        pthread_mutex_unlock(&_k_mutex);
...
void * calculateOutputV(void * arg)
{
    ...
    while ( k <= kf )
    {
        ...
        // Calcule delay
        long int delay = (end.tv_nsec - start.tv_nsec < 0);
        if ( delay < 0 )
        {
            delay = end.tv_nsec + _MAX_TV_NSEC_VALUE - start.tv_nsec;
        }
